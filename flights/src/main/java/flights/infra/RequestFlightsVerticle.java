...
public class RequestFlightsVerticle extends AbstractVerticle {
  ...
  public void start() throws Exception {
    ...
    this.vertx.eventBus().consumer(FLIGHTS_REQUESTER_EB, handler -> {
      final FlightQuery flightQuery = Json
          .decodeValue(handler.body().toString(), FlightQuery.class);
      ...
      webClient.get(target).rxSend().subscribe(bufferHttpResponse -> {
        final FlightResponse flightResponse = Json
            .decodeValue(bufferHttpResponse.bodyAsString(), FlightResponse.class);
